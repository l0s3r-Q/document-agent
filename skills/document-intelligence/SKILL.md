...
description: 文档智能写作代理。理解用户自然语言需求，自动识别文档类型（通用文档/学位论文/党政公文/合同协议/招投标文书/法律文书/政府工作报告/技术文档/简历/通知公告/会议纪要/演讲稿/方案建议书/邀请函/新闻稿/工作总结/产品说明书/验收报告/Excel表格/PDF转换/PPT演示），决策结构策略（从零生成/改结构/不改结构），按规范排版生成 .docx/.xlsx/.pdf/.pptx；支持批量生成、模板导入与管理、结构改造建议。触发词：文档、公文、通知、公告、报告、方案、论文、合同、协议、招标、投标、纪要、会议纪要、演讲稿、演讲、邀请函、起诉状、律师函、简历、政府工作报告、需求说明书、Excel、表格、报表、xlsx、PDF、导出PDF、转换PDF、PPT、演示、幻灯片、pptx、润色、改写、排版、格式、范文、模板导入、批量。
---
...
- 标题黑体 18pt 加粗居中；正文宋体小四 1.5 倍行距
- 要素：标题 → 会议基本信息（时间/地点/主持/参会）→ 议题 → 讨论 → 决议 → 待办 → 落款

### 演讲稿 —— speech
- 标题黑体 18pt 加粗居中；正文宋体小四 1.5 倍行距
- 结构：标题 → 称呼 → 开场 → 引入 → 主体论述（分点）→ 总结 → 致谢

### 方案建议书 —— proposal
- 标题黑体 18pt 加粗居中；正文宋体小四 1.5 倍行距
- 结构：背景 → 目标 → 方案设计 → 实施计划 → 预算 → 预期效果 → 风险对策

### 邀请函 —— invitation
- 标题宋体 18pt 加粗居中；正文宋体小四 1.5 倍行距
- 结构：标题 → 称呼 → 邀请事由 → 活动详情（时间/地点/议程）→ 确认方式 → 落款

### 新闻稿 —— news_release
- 标题黑体 18pt 加粗居中；正文宋体小四 1.5 倍行距
- 结构：标题 → 导语（5W）→ 主体（详情/数据）→ 背景 → 结语

### 工作总结 —— work_summary
- 标题黑体 18pt 加粗居中；正文宋体小四 1.5 倍行距
- 结构：总体概述 → 主要工作（分点+数据）→ 存在问题 → 经验体会 → 下一步计划

### 产品说明书 —— product_manual
- 标题黑体 18pt 加粗居中；正文宋体小四 1.5 倍行距
- 结构：概述 → 技术参数（表格）→ 功能 → 使用方法 → 注意事项 → 维护保养

### 验收报告 —— acceptance_report
- 标题黑体 18pt 加粗居中；正文宋体小四 1.5 倍行距
- 结构：项目概述 → 验收依据 → 内容方法 → 结果（表格）→ 结论 → 附件
